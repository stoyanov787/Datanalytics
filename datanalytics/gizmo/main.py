...
def validate_arguments(expected_arg_count):
...
def handle_data_prep(project_name, timeout):
...
def handle_training(project_name, timeout):
...
def handle_evaluation(project_name, session_id):
    """Handle the evaluation module."""
    logger.info("Starting evaluation module")
    session_path = os.path.join(SESSION_DATA_DIR, f"TRAIN_{project_name}_{session_id}")
    if not os.path.exists(session_path):
        logger.warning(f"Session ID '{session_id}' is not valid")
        raise ValueError("Session ID is not valid")

    output_project_path = os.path.join(SESSION_DATA_DIR, f"EVAL_{project_name}_{session_id}")
    if not os.path.exists(output_project_path):
        os.mkdir(output_project_path)
    ...
    try:
        ...
        if module_type == "--data_prep_module":
            ...
            if sys.argv[4] == "standard" and sys.argv[5] == "--session":
                ...
                handle_evaluation(project_name, session_id)
            else:
